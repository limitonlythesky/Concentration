...
class GameViewController: UIViewController {
    ...
    private let collectionView: UICollectionView = {
        ...
        CV.backgroundColor = Colors.mainBackground
        return CV
    }()
    
    private let resetButton: UIButton = {
        let button = UIButton()
        button.setTitle("Start/Reset", for: .normal)
    ...
    override func viewDidLoad() {
        ...
        APIService.shared.getCardImages { (CardsArray, error) in
            if let _ = error{
                print("Error")
            }
    ...
    private func setActions() {
        resetButton.addTarget(self, action: #selector(handlePlayButton), for: .touchUpInside)
    }
    ...
    private func setupUI(){
        [resetButton, collectionView].forEach{ view.addSubview($0) }
        ...
        view.addConstraint(NSLayoutConstraint(item: collectionView, attribute: .top, relatedBy: .equal, toItem: resetButton, attribute: .bottom, multiplier: 1.0, constant: 10))
...
extension GameViewController: MemoryGameProtocol{
    func memoryGameDidStart(_ game: MemoryGame) {
        Statistics.instance.totalGames += 1
        collectionView.reloadData()
    }
    
    func memoryGameDidEnd(_ game: MemoryGame) {
        Statistics.instance.totalWins += 1
